...
if (!defined('ABSPATH')) exit('No direct script access allowed');
 ...
 /**
 * CodeNegar WooCommerce AJAX Product Filter by Custom Meta widget
 *
...
class Woocommerce_product_filter_custom_slider_meta extends WP_Widget{
    public function __construct() {
        global $codenegar_wcpf;
        parent::__construct(
            'woocommerce_product_filter_custom_slider_meta',
            __('Product Filter by Custom Meta', $codenegar_wcpf->text_domain),
            array('description' => __('Filter products by non-native WooCommerce products meta.', $codenegar_wcpf->text_domain))
        );
    }
    ...
    public function form($instance) { // Backend widget form
		...
		<p>
			<label for="<?php echo $this->get_field_id('parent'); ?>" ><?php _e('Custom Meta Name', $codenegar_wcpf->text_domain); ?>:</label>
			<input
				type="text"
				class="widefat"
				id = "<?php echo $this->get_field_id('parent'); ?>"
				name = "<?php echo $this->get_field_name('parent'); ?>"
				value = "<?php if(isset($parent)) echo $parent; ?>"
			/>
		</p>
...
function woocommerce_product_filter_custom_slider_meta() {
    register_widget('woocommerce_product_filter_custom_slider_meta');
}

add_action('widgets_init', 'woocommerce_product_filter_custom_slider_meta');
?>
