...
foreach( pb_backupbuddy::$options['remote_destinations'] as $destination_id => $destination ) {
	$destinationTabs[] = array(
		'title'		=>		'<span title="Destination type: ' . $destination['type'] . '. ID: ' . $destination_id . '"><img src="' . pb_backupbuddy::plugin_url() . '/destinations/' . $destination['type'] . '/icon50.png" width="16" height="16" style="vertical-align: -2px;"> <span class="destination_title">' . $destination['title'] . '</span></span>',
		'slug'		=>		'destination_' . $destination['type'] . '_' . $destination_id,
	);
}

$destinationTabs[] = array( 'title' => '+ Add New', 'slug' => 'add_new' );
...
foreach( pb_backupbuddy::$options['remote_destinations'] as $destination_id => $destination ) {
	...
	echo '<iframe id="pb_backupbuddy_iframe" src="' . $url . '" width="100%" height="3000" frameBorder="0">Error #4584594579. Browser not compatible with iframes.</iframe>';
	echo '</div>';
