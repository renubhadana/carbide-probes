	...
	<td>
		<?php if ( ! empty( $available_methods ) ) : ?>
			...
			<?php if ( 1 === count( $available_methods ) ) :
                ...
                <?php if ( $method->id === 'user_shipping_method' && ! is_cart() ) : ?>
                    <br />
                    <input type="text" name="shipping_customer_account" />
                <?php endif; ?>
			...
			<?php elseif ( get_option( 'woocommerce_shipping_method_format' ) === 'select' ) : ?>
                ...
                <?php if ( selected( 'user_shipping_method', $chosen_method, false ) && ! is_cart() ) : ?>
                    <input type="text" name="shipping_customer_account" />
                <?php endif; ?>
				...
				<ul id="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<li>
                            ...
                            <?php if ( $method->id === 'user_shipping_method' && checked( 'user_shipping_method', $chosen_method, false ) && ! is_cart() ) : ?>
                                <input type="text" name="shipping_customer_account" />
                            <?php endif; ?>
						</li>
					<?php endforeach; ?>
